...
export default class Network {
  ...
  constructor() {
    ...
    const endpoint = `ws://34.226.248.94:2567`  // Update with your EC2 IP
  ...
  // Add all the existing methods here
  async joinLobbyRoom() {
  ...
  // Add the new sendFile method
  sendFile(file: { fileName: string; fileData: string; fileType: string }) {
    if (!this.room) {
      console.error("Room not initialized. Cannot send file.")
      return
    }
    this.room.send(Message.UPLOAD_FILE, file)
  }
